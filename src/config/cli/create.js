...
async function init() {
  ...
  if (templates.indexOf('app') > -1) await util.createFile(dirPath, 'app.ftl', template.getHTML(getTemplateOption(dependencies, { type: 'app' })));
  if (templates.indexOf('mo') > -1)  await util.createFile(dirPath, 'mo.ftl',  template.getHTML(getTemplateOption(dependencies, { type: 'mo'  })));
  if (templates.indexOf('pc') > -1)  await util.createFile(dirPath, 'pc.ftl',  template.getHTML(getTemplateOption(dependencies, { type: 'pc'  })));
